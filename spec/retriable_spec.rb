  ...
  context '#retriable' do
    ...
    context 'with interval enumerator' do
      ...
      it 'uses size of Enumerator if it can be determined without iterating' do
        expect do
          described_class.retriable(on: StandardError, tries: 6,
                                    intervals: intervals_with_size, max_elapsed_time: 1.2) do
            increment_tries_with_exception
          end
        end.to raise_error(StandardError)
      ...
      it 'recognizes if Enumerator stops iterating' do
        expect do
          described_class.retriable(on: StandardError, tries: 6,
                                    intervals: intervals_with_one_result, max_elapsed_time: 1.2) do
            increment_tries_with_exception
          end
        end.to raise_error(StandardError)
      ...
      it 'lazily iterates through the enumerator' do
        start_time = Time.now
        intervals = Enumerator.new do |result|
          loop do
            result << 0.00001
            sleep 0.2
          end
