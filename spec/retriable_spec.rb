...
describe Retriable do
  ...
  describe "with sleep disabled" do
    ...
    it '#retriable retries with a hash exception where the value is a proc ' \
       'that returns true' do
    ...
    it '#retriable does not retry with a hash exception where the value is a ' \
       'proc that returns false' do
      matcher = ->(e) { e.message == 'something went wrong' }
      tries = 0
      expect do
        subject.retriable on: { TestError => matcher }, tries: 2 do
